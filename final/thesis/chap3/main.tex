...
\proof{
  Given an polynomial-time algorithm with a complexity of $\mathcal{O}(f(n))$ to solve recognition of $G = (V,E,\gamma,\overrightarrow{E})$, we can run again this algorithm by testing every possible orientation of its complement. This algorithm would take $\mathcal{O}(f(n))2^{|E|-1} = \mathcal{O}(f(n)2^{|E|})$ time to execute. \qed
}\\
