...
In the previous section we have shown that MUIG $\subsetneq$ TSG \ref{theo:muigTSG}. We have even shown every forbidden induced subgraph of MUIG in Chapter \ref{chap:interval}. Here we are going to overview these forbidden induced subgraphs and we their inclusion in TSG. \todo{to continue phrasing}
...
Note that if $|u_x - v_x| \leq \alpha_{u,v}$ then $u$ and $v$ are touching.
...
for every $t_y$. We assume that $u_y = 0$ and $u_x = k$ with $k \in \mathbb{R}$ without loss of generality.
