...
\end{proofpart}
\begin{proofpart}
  Now we have to prove that $F$ is a forbidden induced subgraph of TTL. We will try to construct it by taking a induced subgraph that is representable: we take $F_{-1} = F - x$ with $x\in V(F)$. We notice that $V(F_{-1})$ is a chordless $C_{4}$ ($abcd$) with a vertex $e$ adjacent to any two consecutive vertices $x,y \in V(C_4)$ creating the triangle $xye$.\\

  By Lemma \ref{lemma:chordlessLEVEL} we know that $abcda$ is a cycle if $ab$ and $cd$ are level edges. We can classify these vertices in two sets: $\ell(V)$ and $\arr(V)$.
  \todo{Finish proof here}
\end{proofpart}
}

\todo[inline]{Then in the hierarchy, is MUIG $\subsetneq$ TTL or $\subsetneq$ TTL true? }
