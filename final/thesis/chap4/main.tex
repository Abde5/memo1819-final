...
A two-level graph can be defined
...
\end{proofpart}
\begin{proofpart}
  Now we have to prove that $D$ is a forbidden induced subgraph of TTLG. We will try to construct it by taking a induced subgraph that is realizable: we take $D_{-1} = D - x$ with $x\in V(D)$. We notice that $V(D_{-1})$ is a chordless $C_{4}$ ($abcd$) with a vertex $e$ adjacent to any two consecutive vertices $x,y \in V(C_4)$ creating the triangle $xye$.
