...
\prop {An $\sigma$-SG($c$) graph $G$ (with $c < 1$) can be characterized by computing $\delta : A \times B \to E$ where $A,B \subseteq G$ and $A \cup B = \varnothing$:
...
\todo[inline]{FALSE PROOF/THEOREM -> TO MAKE NEW STATEMENT}
...
We can prove that $\sigma$-SG($\epsilon$) $\neq$ TSG because we can construct a graph $G$ such that $G \in$ TSG and $G \notin$ $\sigma$-SG($\epsilon$). To construct this graph we take a clique $K_n$. Then, we take two disjoint vertices $x$ and $y$ and we add the edges $\forall c \in K_{n-1} : xc, yc \in E$ with $K_{n-1} \in K_{n}$. (informally: we attach 2 vertices to every vertex of the clique except 2... )
