...
In this chapter I define thin two-level graphs as a two-level graph where $L = \{0,\epsilon\}$ where $\epsilon$ is an arbitrarily small real number.
...
\lemma{$\phi$ is a $\sigma(\epsilon)$-realization of $C_4$ if and only if:
\begin{itemize}
  \item There are two disks on each line.
  \item The two (left-)rightmost disks touch.
\end{itemize}
}

\proof{
  First, we prove that $\phi$ cannot be a $\sigma(\epsilon)$-realization if more than two disks are on the same line:
  \begin{itemize}
    \item \textbf{4 disks on the same line:} When every center of unit disks is located in the same line it is considered an UIG.
    This is equal to stating that $C_4 \in$ UIG, which is impossible by Theorem \ref{theo:intervalChord}.
    \item \textbf{3 disks on the same line:} Let $G = C_4$. We define $a,b,c \in V(G)$ as the group of three consecutive points in one line ($y=0$). We know that $c_x > a_x+1$; $ac \notin E(G)$ and $a_x < b_x < c_x$ ($a_x$ is the leftmost vertex and $c_x$ the rightmost one).

    We have $d \in V(G)$ and ${ad,cd} \in E(G)$ to complete $C_4$. Because $bd \notin E(G)$, $d_x \in \mathbb{R}\setminus [b_x-\sqrt{1-\epsilon^2},b_x+\sqrt{1-\epsilon^2}]$.\\
    ...
    Therefore, we cannot realize $C_4$ with three points in the same line.
  \end{itemize}
  And because we prove
