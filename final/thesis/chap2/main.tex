...
\end{tikzpicture}
\end{scaletikzpicturetowidth}
\caption{A Venn diagram of four sets. You may notice that some subsets are not represented here (\textit{e.g.} $v_2v_4$ or $v_1v_3$.)}
\label{fig:uuigudg}
\end{figure}
...
\proof{The UUIG recognition of a graph $G$ is in $\mathcal{NP}$ because we can build a \emph{polinomial time verifier} that takes a level structure of $G$ and check whether each level is a clique or not. Viceversa, we can build another one that takes a separation and check whether each clique is a level of a level structure.}
