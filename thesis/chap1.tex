...
\begin{defn}
  ...
  adjacent. This is denoted $K_n$ with $n$ the number of elements. If $G$ is an
  induced graph of $H$ then $G$ is a clique of $H$.
\end{defn}

\begin{defn}
  A graph $G$ is bipartite if there are two disjoint independent sets $A,B \subset V$
  such that every vertex of $A$ connects to one in $B$.
\end{defn}
...
Unlike for UIG class, $K_{1,3}$ is a MUIG as seen in figure \ref{fig:muigK13}.

\subsubsection{Disks}

A unit disk graph $G$ is a graph that is an intersection graph of unit
disks on the plane. This class of graphs
